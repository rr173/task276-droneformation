...
func TestConcurrentIntentWritesSerialized(t *testing.T) {
	app := newProbeApp(t)
	runID, ids := mustRunAC(t, app, 20)
	const workers = 20
	...
	for i := 0; i < workers; i++ {
		...
		go func(i int) {
			defer wg.Done()
			_, err := app.IngestIntent(context.Background(), runID, ids[i], farIntent(1, float64(i)*10))
			if err != nil {
				errCh <- fmt.Errorf("ac %d: %w", i, err)
			}
		}(i)
	}
	wg.Wait()
	...
	intents, err := app.ListIntentsByRun(runID)
	if err != nil {
		t.Fatal(err)
	}
	if len(intents) != workers {
		t.Fatalf("want %d intents, got %d", workers, len(intents))
	}
}
