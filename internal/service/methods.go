...
func (a *App) RegisterAircraft(ctx context.Context, runID, callsign string, radiusM, heightBaselineM float64) (*model.Aircraft, error) {
	...
	if _, err := a.store.GetRun(runID); err != nil {
		return nil, fmt.Errorf("register aircraft: %v", err)
	}
	if radiusM <= 0 {
	...
	if err := a.store.CreateAircraft(ac); err != nil {
		return nil, fmt.Errorf("create aircraft: %v", err)
	}
	return ac, nil
}
...
func (a *App) IngestIntent(ctx context.Context, runID, aircraftID string, in IntentInput) (*model.IntentSegment, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ingest get run: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ingest get aircraft: %v", err)
	}
	if ac.RunID != runID {
	...
	if err := a.store.PersistIntentBundle(store.IntentBundle{
		...
		return nil, fmt.Errorf("persist intent bundle: %v", err)
	}
	return it, nil
}
...
func (a *App) createSnapshotAndRels(ctx context.Context, run *model.FormationRun, rels []model.AvoidanceRelation, patches []store.IntentStatusPatch) (int64, string, error) {
	...
	if err != nil {
		return 0, "", fmt.Errorf("persist verification: %v", err)
	}
...
func (a *App) VerifyRun(ctx context.Context, runID string) (*VerificationResult, error) {
	...
	run, err := a.store.GetRun(runID)
	if err != nil {
		return nil, fmt.Errorf("verify get run: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("verify list aircraft: %v", err)
	}
	...
	for _, ac := range acs {
		...
		if err != nil {
			return nil, fmt.Errorf("verify list intents: %v", err)
		}
		...
		if covErr != nil {
			return nil, fmt.Errorf("verify covariance: %v", covErr)
		}
...
func (a *App) PublishSnapshot(ctx context.Context, runID string, snapID int64) error {
	...
	run, err := a.store.GetRun(runID)
	if err != nil {
		return fmt.Errorf("publish get run: %v", err)
	}
	if state.IsSealed(*run) {
		return model.ErrRunSealed
	}
	...
	if err != nil {
		return fmt.Errorf("publish get snapshot: %v", err)
	}
	if snap.RunID != runID {
	...
	if err := a.store.PersistPublish(runID, snapID, snap.Status, a.now()); err != nil {
		return fmt.Errorf("persist publish: %v", err)
	}
	return nil
}
...
func (a *App) IsolateAircraft(ctx context.Context, runID, aircraftID string) error {
	...
	run, err := a.store.GetRun(runID)
	if err != nil {
		return fmt.Errorf("isolate get run: %v", err)
	}
	if state.IsSealed(*run) {
		return model.ErrRunSealed
	}
...
func (a *App) ReinstateAircraft(ctx context.Context, runID, aircraftID string) error {
	...
	run, err := a.store.GetRun(runID)
	if err != nil {
		return fmt.Errorf("reinstate get run: %v", err)
	}
	if state.IsSealed(*run) {
		return model.ErrRunSealed
	}
	ac, err := a.store.GetAircraft(aircraftID)
	if err != nil {
		return fmt.Errorf("reinstate get aircraft: %v", err)
	}
	if ac.RunID != runID {
		return model.ErrAircraftNotFound
	}
	if err := a.store.PersistReinstate(aircraftID); err != nil {
		return fmt.Errorf("persist reinstate: %v", err)
	}
	return nil
}
...
func (a *App) GetStats(runID string) (*RunStats, error) {
	...
	acs, err := a.store.ListAircraft(runID)
	if err != nil {
		return nil, fmt.Errorf("stats aircraft: %v", err)
	}
	intents, err := a.store.ListIntentsByRun(runID)
	if err != nil {
		return nil, fmt.Errorf("stats intents: %v", err)
	}
	snaps, err := a.store.ListSnapshots(runID)
	if err != nil {
		return nil, fmt.Errorf("stats snapshots: %v", err)
	}
	conf := 0
	relCount := 0
	relSnapID := int64(0)
	if pub, pubErr := a.store.LatestPublishedSnapshot(runID); pubErr != nil {
		return nil, fmt.Errorf("stats published: %v", pubErr)
	} else if pub != nil {
		relSnapID = pub.ID
	} else if len(snaps) > 0 {
	...
	if relSnapID > 0 {
		...
		if relErr != nil {
			return nil, fmt.Errorf("stats relations: %v", relErr)
		}
		relCount = len(rels)
		for _, r := range rels {
			if r.Status == model.RelationInsufficient {
